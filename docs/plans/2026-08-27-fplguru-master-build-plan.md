...
> Remaining sub-plans after this override: **2** (F + all of Phase 1 + **P1i** design system + **P2h/P2i/P2e/P2a/P2b/P2c/P2d/P4b/P2f/P4a** done; P1g + P4c dropped). **Decided:** xG source → PitchAPI; LLM → Google Gemini + `$5/mo` cap; **UI is now a dark design system + sidebar app shell** (P1i). **Telegram (P2g) deferred.** **All unblocked sub-plans are shipped.** Everything left is **blocked on external keys/decisions:** P3a (YouTube API key) → P3c; P3b (press-conference source); P3d (WhatsApp BSP + Meta verification); P1a-auth (OAuth creds / email transport). P2g (Telegram) deferred by user.
...
| **P2b** ✅ | Advanced xP Engine — *done* ([`2026-08-28-p2b-advanced-xp.md`](2026-08-28-p2b-advanced-xp.md), branch `feature/p2b-advanced-xp`) | Hand-rolled pure-numpy **GBRT** per position (`fplguru_ml.gbrt`; LightGBM blocked by Smart App Control) + two quantile GBRTs per position for floor/ceiling bands; `AdvancedXP` bundle (`adv-v1`, `packages/ml/artifacts/advanced/*.json`, ~660 KB committed); 5 xG features (`form_xg_5`/`form_xa_5`/`xg_overperf_5`/`form_xgc_5`/`form_ict_5`) trained from historical `merged_gw` CSVs — **0.0 at serve time until `player_xg` has an FPL id map**, GBRT still predicts from the 9 shared features; `walk_forward_adv` (strided) + `scripts/train_adv_xp.py`/`backtest_adv_xp.py` → [`docs/xp-backtest/adv-2026-08-28.md`](../xp-backtest/adv-2026-08-28.md): **adv beats basic RMSE on 4/4 positions** (M3 ✓); worker `compute_xp` writes both tiers with an approximate `x_*` split; API `?model=auto\|basic\|advanced` on `/xp`, `/players/{id}/xp`, `/entries/{id}` (+ resolver threaded through `/overpowered`, `/entries/{id}/captain`); web squad model toggle. Deferred: real component models, SHAP/tree-path attribution, isotonic calibration, multi-seed ensemble, full compounding rollout. **Unblocks P2c.** | P1c, P2a | §5.2 (Advanced), §5.4, §5.5 |
| **P2c** ✅ | LLM Explanation Layer — *done* ([`2026-08-28-p2c-llm-explanation.md`](2026-08-28-p2c-llm-explanation.md), branch `feature/p2c-llm-explanation`) | `AdvancedXP.explain_row` = **occlusion-at-median** local attribution (SHAP stand-in) + persisted per-position feature medians in `adv-v1` `meta.json`; new pure pkg **`fplguru-explain`** (`explanation_prompt` / `template_explanation` / `DRIVER_PHRASES`); shared `fplguru_ml.serving.{to_adv_row,adv_feature_row}` (worker + API); `xp_rationale` cache table (`0012`); `GET /players/{id}/xp/explain?horizon=&model=` → `{drivers[], text, source}` via `generate_within_budget` (`xp_explain`, cached, template fallback); web squad per-row "Why?" panel with up/down driver chips. Deferred: proper SHAP / tree-path attribution, per-GW drivers, worker pre-warm of rationales, surfacing on `/captain` + player detail. | P2b | §5.4, §5.2 |
| **P2d** ✅ | Optimize My Team — *done* ([`2026-08-28-p2d-optimize-my-team.md`](2026-08-28-p2d-optimize-my-team.md), branch `feature/p2d-optimize`) | pure pkg **`fplguru-optimize`** — `best_xi` (formation-aware XI + captain/vice + bench, reserve GK first), `suggest_transfers` (greedy, 4-pt hit accounting, £bank + same-position + max-3-per-club, one plan per k = 0..max), `chip_hints` (DGW → bench-boost/triple-captain, BGW → free-hit); `GET /entries/{id}/optimize?horizon=1–10&max_transfers=&free_transfers=&model=` (adv xP, top-40-per-position market, bank from `EntryGwHistory`, `gw_calendar` reused); `/optimize` web page (horizon + max-transfers selectors via prefs, recommended-move card, chip chips, XI table + bench). No tier gate (2026-08-27 pivot). Deferred: global/ILP optimiser, cross-position transfers, multi-week planning, price-change modelling, true chip simulation. **Unblocks P4a.** | P1c, P2b, P2c | §4.5 |
...
| **P2f** ✅ | H2H Match Helper — *done* ([`2026-08-28-p2f-h2h-match-helper.md`](2026-08-28-p2f-h2h-match-helper.md), branch `feature/p2f-h2h`) | pure pkg **`fplguru-h2h`** — `compare_squads` (best-XI margin on adv xP via `fplguru_optimize.best_xi`, differentials both ways, captain divergence, shared count) + `template_strategy` (ahead/behind/level, flags their top differential); `GET /entries/{id}/h2h/{opponent_id}?horizon=&model=` syncs the opponent via `sync_entry` (works for any FPL entry id) then compares; `/h2h` web page (opponent-id input, margin + strategy card, captain chips, side-by-side differential tables). No tier gate (2026-08-27 pivot). No migration. Deferred: LLM strategy, bench/auto-sub modelling, live in-GW tracking, H2H-league auto-pairing, short-TTL opponent-sync guard. | P2b | §4.12 |
...
| # | Sub-plan | Delivers | Depends on | PRD refs |
|---|---|---|---|---|
| **P4a** ✅ | Saved Optimization Plans — *done* ([`2026-08-28-p4a-saved-optimization-plans.md`](2026-08-28-p4a-saved-optimization-plans.md), branch `feature/p4a-saved-plans`) | `optimization_plan` table (`0013`, JSON-text `payload` snapshot); `entry_optimize` body factored into `_run_optimize`; `POST/GET/DELETE /entries/{id}/plans[/{plan_id}]` — save a named snapshot (capped at `saved_plans_cap`, default 5, oldest evicted), list summaries, fetch one full plan, delete; all scoped to the linked team. `/optimize` page: name + "Save current", saved-plans list with Open (reloads snapshot + syncs selectors) / Delete. No tier gate (2026-08-27 pivot). Deferred: plan diffing, per-GW auto-snapshot, share links, squad-at-time drift. | P2d | §4.5, §3 |
